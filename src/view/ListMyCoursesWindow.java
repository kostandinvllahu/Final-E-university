...
public class ListMyCoursesWindow extends Window {

	private static final long serialVersionUID = 6056219387065363051L;

	public ListMyCoursesWindow() {

		ArrayList<Course> list = DBHandler.db.getMyCourses();
		
		if (list.size() == 0) {
			JOptionPane.showMessageDialog(null, "No data!");
			throw new IllegalArgumentException() {
				private static final long serialVersionUID = 9039115680676127452L;
				@Override
		...
		for (Course course : list) {
			...
			data[i][7] = new CourseButton("", course) {
		...
		table.getColumn(" ").setCellEditor(new CourseButtonEditor(new JCheckBox()) {
			private static final long serialVersionUID = -6538101143346750946L;
			@Override
			public Object getCellEditorValue() {
				if (isPushed) {
					DBHandler.db.deleteCourseFromMyList(((CourseButton) button).getCourse());
					setVisible(false);
				}
				isPushed = false;
				return new String(label);
