...
public class AddCourseRatingWindow extends Window {

	private static final long serialVersionUID = -1284586862899925296L;

	public AddCourseRatingWindow() {
		...
		table.getColumn(" ").setCellEditor(new CourseButtonEditor(new JCheckBox()) {
			...
			public Object getCellEditorValue() {
				if (isPushed) {
					setVisible(false);
					new AddRatingWindow(((CourseButton) button).getCourse()).setVisible(true);
				}
				isPushed = false;
				return new String(label);
			}
